...
2. Дважды кликните по `LegacyDownloader-GUI.bat`.
...
> Если ваш антивирус отмечает `rclone.exe` (файл в папке `bin`), смотрите
...
## 3. Проверка требований к ПО

Legacy нужно несколько дополнительных программ, которых нет в Windows по
умолчанию — Kinect SDK и среды выполнения Visual C++. Legacy Downloader
проверяет это автоматически, один раз:

- **Если игра у вас уже была**, эта проверка происходит сразу, ещё до
  появления главного окна.
- **Если вы выбрали «Загрузите для меня»**, она происходит автоматически
  сразу после завершения загрузки выше.

![Диалог требований к ПО](images/07-requirements-ru.png)

Всё отмеченное красным отсутствует — нажмите **Установить** рядом с
пунктом, чтобы скачать и установить его, или нажмите на название-ссылку,
чтобы вместо этого открыть официальную страницу загрузки Microsoft. Когда
всё станет зелёным, нажмите **Закрыть**, чтобы продолжить. Вы можете
снова открыть эту проверку в любой момент через кнопку **Требования** в
главном окне.
...
- **Песни** — выберите:
  ...
  - **Определённые** — вместо этого выберите именно то, что нужно.
    Нажмите **«Выбрать карты / песни»**, чтобы открыть выбор — об этом в
    следующем шаге.
...
- **Требования** — снова открывает проверку из предыдущего шага, когда бы
  вы ни захотели перепроверить или установить что-то пропущенное.

## 5. Выбор отдельных песен

Нажмите **«Выбрать карты / песни»** (в главном окне, в любой момент), чтобы
открыть выбор:

![Выбор песен и изданий](images/06-songbrowser-ru.png)

- **Издания**, слева — отметьте флажок целого издания, чтобы получить всё
  из него. Наполовину заполненный флажок означает, что выбраны только
  некоторые песни этого издания.
- **Песни**, справа — каждая отдельная песня со своим изданием,
  сложностью и нагрузкой (интенсивностью тренировки), если они известны.
  Отмечайте или снимайте отметку с любой песни отдельно — не нужно брать
  сразу всё издание.
- **Поиск** — введите название, исполнителя или кодовое имя, чтобы мгновенно
  отфильтровать список.
- **Фильтры** — сузьте список дальше по Сложности или Нагрузке с помощью
  выпадающих меню в правом верхнем углу.
- **Отметить все показанные / Снять отметки со всех показанных** —
  выберите массово всё, что показывает текущий поиск или фильтр, вместо
  того чтобы кликать по каждой песне отдельно.
- **Столбцы...** — покажите или скройте столбцы Исполнитель, Сложность или
  Нагрузка для более простого вида.

Нажмите **ОК**, чтобы сохранить выбор, или **Отмена**, чтобы вернуться без
изменений.

> Песни, для которых в списке сообщества ещё нет названия, всё равно
> отображаются (просто с именем файла вместо названия) — они всё равно
> скачаются и будут работать, просто без понятного названия, пока кто-то
> его не добавит.
...
## 8. Возвращение позже за новыми песнями

Просто запустите `LegacyDownloader-GUI.bat` снова в любое время. Программа
...
- **Добавить или убрать песни**: снова нажмите **«Выбрать карты /
  песни»**, отметьте или снимите отметки с того, что хотите, и нажмите ОК.
  Снятие отметки с уже загруженного — целого издания или всего нескольких
  песен — спросит, удалить ли также эти файлы или просто прекратить
  получать для них обновления, сохранив то, что у вас уже есть.
...
- **Антивирус помещает `rclone.exe` в карантин или удаляет его** —
  ...
  разрешите, затем снова запустите `LegacyDownloader-GUI.bat`.
- **При двойном клике по `LegacyDownloader-GUI.bat` появляется
...
- **Написано «актуально», но песен не хватает** — откройте **«Выбрать
  карты / песни»** и проверьте, что вы действительно отметили нужные (или
  выберите **«Всё»**).
