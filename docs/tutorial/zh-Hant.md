...
其他語言：[English](en.md) · [Français](fr.md) · [Español](es.md) · [Filipino](fil.md) ·
...
Legacy 需要一些 Windows 沒有內建的額外軟體——Kinect SDK 和 Visual C++
執行環境。Legacy Downloader 會自動檢查一次：

- **如果您本來就已經有這個遊戲**，這項檢查會立即進行，甚至在主視窗出現
  之前。
- **如果您選擇了「請為我下載」**，則會在上面的下載完成後自動進行。

![軟體需求對話框](images/07-requirements-zh-Hant.png)

標紅的項目表示缺少——點擊旁邊的**安裝**即可下載並安裝，或者點擊帶連結的
名稱，改為開啟微軟官方的下載頁面。全部變綠後，點擊**關閉**繼續。您之後
隨時可以透過主視窗上的**系統需求**按鈕重新開啟這項檢查。
...
- **歌曲** — 選擇：
  ...
  - **特定** — 精確選擇您想要的內容。點擊**「選擇地圖 / 歌曲」**開啟選擇器
    ——下一步會詳細介紹。
- **「下載 / 檢查更新」** — 那個大按鈕。點擊它來取得您選擇的內容，之後隨時
  再次點擊即可檢查新歌曲或更新。
- **系統需求** — 重新開啟上一步的檢查，方便您隨時重新檢查或安裝之前跳過
  的項目。
...
![歌曲和版本選擇器](images/06-songbrowser-zh-Hant.png)

- **版本**（左側）——勾選整個版本的核取方塊即可取得其中的全部內容。半選
  狀態的核取方塊表示該版本只有部分歌曲被選取。
- **歌曲**（右側）——每首歌曲及其版本、難度和強度（已知時會顯示）。您
  可以單獨勾選或取消勾選任意一首歌曲——無需一次取得整個版本。
- **搜尋**——輸入標題、藝人或代號即可立即篩選清單。
- **篩選**——使用右上角的下拉選單依難度或強度進一步縮小清單範圍。
- **全選顯示項 / 取消全選顯示項**——一鍵選取目前搜尋或篩選結果中顯示的
  所有內容，而不必逐首點擊。
- **欄...**——顯示或隱藏藝人、難度或強度欄，以取得更簡潔的檢視。

點擊**確定**儲存您的選擇，或點擊**取消**放棄變更返回。

> 社群清單中尚未命名的歌曲同樣會顯示出來（只是用檔名代替標題）——
> 它們仍會正常下載並可以使用，只是在有人新增親切名稱之前暫時沒有名字。
...
- **新增或移除歌曲**：再次點擊**「選擇地圖 / 歌曲」**，勾選或取消勾選您想要
  的內容，然後點擊確定。取消勾選已下載的內容（無論是整個版本還是僅幾首
  歌曲）時，會詢問您是要同時刪除這些檔案，還是只是停止更新但保留現有內容。
...
- **點擊資料夾按鈕沒有反應** — 選擇視窗可能開啟在了主視窗*後面*；請查看
  您的工作列。
- **顯示「已是最新」但我缺少歌曲** — 開啟**「選擇地圖 / 歌曲」**，檢查您是
  否真的勾選了想要的內容（或者選擇**「全部」**）。
