...
Legacy에는 Windows에 기본으로 포함되지 않은 몇 가지 추가 소프트웨어 —
Kinect SDK와 Visual C++ 런타임 — 가 필요합니다. Legacy Downloader가 이를
자동으로, 한 번만 확인합니다:

- **이미 게임이 있었다면**, 이 확인은 메인 창이 나타나기도 전에 바로
  진행됩니다.
- **"다운로드해 주세요"를 선택했다면**, 대신 위의 다운로드가 끝난 직후
  자동으로 진행됩니다.

![소프트웨어 요구 사항 대화 상자](images/07-requirements-ko.png)

빨간색으로 표시된 항목은 없는 것입니다 — 옆의 **설치**를 클릭해 다운로드
및 설치하거나, 링크로 된 이름을 클릭해 Microsoft 공식 다운로드 페이지를
대신 열 수 있습니다. 모두 초록색이 되면 **닫기**를 클릭해 계속 진행하세요.
이 확인은 메인 창의 **필수 요소** 버튼으로 언제든 다시 열 수 있습니다.
...
- **노래** — 선택하세요:
  - **전부** — 사용 가능한 모든 Just Dance 에디션을 받고, 나중에 추가되는
    ...
    고르세요.
  - **특정만** — 대신 원하는 것만 정확히 고르세요. **"맵 / 노래 선택"**을
    클릭해 선택 창을 여세요 — 다음 단계에서 다룹니다.
- **"다운로드 / 업데이트 확인"** — 큰 버튼입니다. 클릭하면 선택한 것을
  받아오고, 나중에 다시 클릭하면 새 노래나 업데이트를 확인할 수 있습니다.
- **필수 요소** — 이전 단계의 확인을 다시 엽니다. 건너뛴 항목을 다시
  확인하거나 설치하고 싶을 때 언제든 사용하세요.

## 5. 개별 노래 선택하기

메인 창에서 언제든지 **"맵 / 노래 선택"**을 클릭하면 선택 창이 열립니다.

![노래 및 에디션 선택 창](images/06-songbrowser-ko.png)

- **에디션** (왼쪽) — 에디션 전체의 체크박스를 체크하면 그 안의 모든 것을
  받습니다. 체크박스가 반쯤 채워져 보이면 해당 에디션의 일부 노래만
  선택된 것입니다.
- **노래** (오른쪽) — 각 노래가 에디션, 난이도, 강도(운동 강도, 알려진
  경우)와 함께 표시됩니다. 노래를 하나씩 개별적으로 체크/해제할 수
  있습니다 — 에디션 전체를 한 번에 받을 필요가 없습니다.
- **검색** — 제목, 아티스트 또는 코드명을 입력하면 목록이 즉시 필터링됩니다.
- **필터** — 오른쪽 상단의 드롭다운으로 난이도나 강도에 따라 목록을 더
  좁힐 수 있습니다.
- **표시된 항목 모두 선택 / 표시된 항목 모두 선택 해제** — 노래를 하나씩
  클릭하는 대신, 현재 검색이나 필터에 표시된 항목을 한 번에 선택합니다.
- **열...** — 아티스트, 난이도, 강도 열을 표시하거나 숨겨서 더 간단한
  화면으로 볼 수 있습니다.

**확인**을 클릭해 선택 내용을 저장하거나, **취소**를 클릭해 아무것도
바꾸지 않고 돌아갑니다.

> 커뮤니티 목록에 아직 이름이 없는 노래도 표시됩니다(제목 대신 파일명으로
> 표시됨) — 누군가 이름을 추가하기 전까지 친숙한 이름이 없을 뿐, 정상적으로
> 다운로드되고 작동합니다.
...
- **노래 추가/제거**: **"맵 / 노래 선택"**을 다시 클릭해 원하는 대로 체크
  하거나 해제한 뒤 확인을 클릭하세요. 이미 다운로드한 것(에디션 전체든
  일부 노래든)의 체크를 해제하면 해당 파일도 삭제할지, 아니면 갖고 있는
  것은 유지하되 업데이트만 중단할지 물어봅니다.
...
- **"최신 상태"라고 나오는데 노래가 부족함** — **"맵 / 노래 선택"**을 열어
  원하는 것에 정말 체크되어 있는지 확인하세요(또는 **"전부"**를 선택
  하세요).
