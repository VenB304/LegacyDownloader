...
2. `LegacyDownloader-GUI.bat` をダブルクリックします。
...
3. 展開したフォルダを開き、**`LegacyDownloader-GUI.bat`** をダブルクリックします。

![展開したフォルダの内容](images/01-extracted-folder.png)

> ウイルス対策ソフトが `rclone.exe`(`bin` フォルダ内のファイル)を検出した場合は、
...
Legacyには、Windowsに標準で入っていない追加のソフトウェア — Kinect SDK と
Visual C++ ランタイム — が必要です。Legacy Downloader はこれを自動で、
一度だけ確認します。

- **すでにゲームを持っていた場合** — この確認はメインウィンドウが表示される
  前に、すぐに行われます。
- **「ダウンロードしてほしい」を選んだ場合** — 代わりに、上記のダウンロードが
  終わった時点で自動的に行われます。

![ソフトウェア要件ダイアログ](images/07-requirements-ja.png)

赤色で表示されているものは未導入です — 横の**インストール**をクリックする
とダウンロードしてインストールできます。または、リンクになっている名前を
クリックすると、代わりに Microsoft 公式のダウンロードページが開きます。
すべて緑色になったら**閉じる**をクリックして進みます。この確認はいつでも
メインウィンドウの**必要条件**ボタンから開き直せます。
...
- **曲** — 選択します。
  - **すべて** — 利用可能なすべての Just Dance エディションを取得し、後で
    ...
    選択肢です — これを選んでください。
  - **特定のみ** — 代わりに欲しいものだけを正確に選びます。**「マップ / 曲
    を選択」**をクリックしてピッカーを開きます — 次のステップで説明します。
- **「ダウンロード / 更新を確認」** — 大きなボタンです。クリックすると
  選んだものを取得し、後で再度クリックすると新しい曲や更新を確認できます。
- **必要条件** — 前のステップの確認を開き直します。見送った項目を
  再確認・インストールしたいときにいつでも使えます。
...
![曲とエディションのピッカー](images/06-songbrowser-ja.png)

- **エディション**(左側) — エディション全体のチェックボックスにチェックを
  入れると、その中身をすべて取得します。チェックボックスが半分埋まって
  表示されている場合、そのエディションの一部の曲だけが選択されています。
- **曲**(右側) — 各曲がエディション、難易度、運動量(わかっている場合)
  とともに表示されます。曲を1曲ずつ個別にチェック/解除できます — エディ
  ション全体をまとめて取得する必要はありません。
- **検索** — タイトル、アーティスト、またはコード名を入力すると、リストが
  即座に絞り込まれます。
- **フィルター** — 右上のドロップダウンで難易度や運動量によってさらに
  リストを絞り込めます。
- **表示中をすべてチェック / 表示中をすべて解除** — 曲を1曲ずつクリックする
  代わりに、現在の検索やフィルターに表示されているものをまとめて選択でき
  ます。
- **列...** — アーティスト、難易度、運動量の列を表示/非表示にして、
  よりシンプルな表示にできます。

**OK** をクリックして選択内容を保存するか、**キャンセル**をクリックして
何も変更せずに戻ります。

> コミュニティのリストにまだ名前が登録されていない曲も表示されます
> (タイトルの代わりにファイル名で表示されます) — 誰かが名前を追加する
> までは親しみやすい名前がないだけで、問題なくダウンロードされ、正常に
> 動作します。
...
いつでも `LegacyDownloader-GUI.bat` を再度実行するだけです。フォルダと曲の選択
内容は記憶されており、**「ダウンロード / 更新を確認」**をクリックすると、
前回の訪問以降に追加されたものをすべて取得できます。

- **曲の追加・削除**: 再度**「マップ / 曲を選択」**をクリックし、好きなように
  チェックを入れたり外したりして、OK をクリックします。すでにダウンロード
  済みのもの(エディション全体でも数曲だけでも)のチェックを外すと、その
  ファイルも削除するか、手元に残したまま更新だけ停止するかを尋ねられます。
...
- **ウイルス対策ソフトが `rclone.exe` を隔離または削除する** — よくある
  ...
  復元して許可し、`LegacyDownloader-GUI.bat` を再度実行してください。
- **`LegacyDownloader-GUI.bat` をダブルクリックするとセキュリティの警告が
...
- **フォルダのボタンをクリックしても何も起こらない** — 選択ウィンドウが
  ...
  確認してください。
- **「最新」と表示されるのに曲が足りない** — **「マップ / 曲を選択」**
  を開いて、本当に欲しいものにチェックが入っているか確認してください
  (または**「すべて」**を選んでください)。
